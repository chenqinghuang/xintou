...
jQuery.validator.addMethod("incomeWy", function (value, element) {
	var income = /(^[0-9]([0-9]+)?(\.[0-9]{1,4})?$)|(^(0){1}$)|(^[0-9]\.[0-9]([0-9])?$)/;
	return this.optional(element) || (income.test(value));
}, "金额格式不对");
...
jQuery.validator.addMethod("helpNeedSix",function(value, element){
    var returnVal = false;
    $(".helpNeedSix").each(function(){
    	if($(this).is(":checked")){
    		returnVal=true;
    	}
    });
